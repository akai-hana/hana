...
| Floating | `src/window/modules/floating.zig` | window.zig, input.zig, bar.zig |
| Fullscreen | `src/window/modules/fullscreen.zig` | window.zig, pipeline.zig, events.zig, actions.zig |
| Minimize | `src/window/modules/minimize.zig` | window.zig, bar.zig, input.zig |
| Workspaces | `src/window/modules/workspaces.zig` | window.zig, input.zig, bar.zig, segment.zig |
...
The behaviors (floating, fullscreen, minimize, workspaces) are imported by several files outside `window/modules/`. Under fact ownership:
...
src/
 ...
 modules/ # each calls core action surface; core owns the facts
 bar/
