...
- **Behavior is frozen.** The contract is `dev/harness/` (21 scenarios,
  golden artifacts) + `src/test/` (107 tests) + `dev/scripts/check-layers.sh`
  (layer rules). Any proposal must name which gate would catch its
  regressions. Verification commands:
  - `zig build && zig build test --summary all` (107/107 expected)
  ...
  - `zig build && bash dev/harness/run-scenario.sh --compare S01-spawn-tiled
    ...
    S17-hover-focus S18-ewmh-fullscreen S19-fullscreen-toggle S20-layout-storm
    S21-hints-resize` (21/21 PASS expected)
- **Layer rules** (enforced by rule 1 of check-layers): raw xcb calls may
  appear ONLY in `src/core/sync/wire.zig`. Model (`src/core/model`) and tiling
  (`src/tiling`, minus facade) are pure - no XCB, no global state reads.
  `src/core/sync/sync.zig` is the SOLE writer of geometry/map/stack/border.
...
Files: `src/core/model/model.zig` (521), `src/core/model/store.zig` (91)
...
Files: `src/core/sync/sync.zig` (430), `src/core/sync/wire.zig` (105)
...
Files: `src/window/actions.zig` (720),
       `src/window/modules/{floating,fullscreen,minimize,workspaces}.zig`
       (283/381/36/138)
...
Files: `src/input/input.zig` (762), `xkbcommon.zig` (237)
...
Sensitivities: slot relay exists to avoid redundant queries during drags - prior analysis suspected removable (direct queries sub-ms at real N) but
...
rotation of long window lists (carousel.zig is a 1-line re-export shim - investigate why). Clock is threadless (deadline math + lazy strftime),
...
Role: in-bar command prompt with vim-style motions/editing (user feature - removal off the table), null_vim stub for feature-off builds.
...
- P3 single writer: all wire mutation confined to src/core/sync/ (+ allowlist);
...
- Harness normalizer determinism, golden diff excludes, per-scenario
  ...
  they pattern on.
- Unit suite green at time of pass: 107/107 across 10 binaries (model 38,
  tiling 15, schema 11, sync 9, perf 9, carousel 9, parser 6, config 5,
  workspaces 3, clock 2).
