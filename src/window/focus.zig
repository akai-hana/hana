...
const State = struct {
    ...
    //
    // tiling_op_cookie
    //   Pending "has the server caught up" round trip from
    //   beginTilingOpSettle(), used to defer re-enabling EnterNotify
    //   suppression-lifting until any crossing events generated by a tiling
    //   reflow are guaranteed to have already been delivered and filtered.
    //   See beginTilingOpSettle's doc comment for the full rationale.
    //
...
/// EnterNotify suppression until it is safe to do so.
///
/// Used by tiling operations that must NOT re-sync focus to wherever the
/// pointer ends up afterward (e.g. toggle_floating_window's keybind path,
/// via withTilingGrabKeepFocus) — unlike beginPointerSync, this never calls
/// setFocus itself.
///
/// Why this exists: a tiling reflow's configure_window calls are only sent
/// to the X server in the caller's closing ungrabAndFlush, at which point
/// the server may generate real EnterNotify events for windows whose edges
/// slid under a stationary cursor (e.g. a neighbour window resizing to make
/// room for the one just toggled into the tiling pool). If suppression were
/// cleared synchronously right after queuing those requests — before they
/// are even flushed — those crossing events would arrive un-suppressed and
/// silently reassign keyboard focus via the ordinary hover-focus path. A
/// second toggle_floating_window keypress (autorepeat, a fast double-tap)
/// would then land on that other window instead of the one just acted on.
///
/// xcb_get_input_focus is queued in the SAME flush as the reflow's
/// configure_window calls. X guarantees that, on a single connection, any
/// event generated by a request is delivered before the reply to a later
/// request — so by the time drainTilingOpSettle() consumes this reply, any
/// crossing event the reflow could have generated is already sitting in
/// (or has already been drained from) the client's event queue. Because
/// handleXcbEvents() drains all pending events before running this
/// function, that event is guaranteed to have been dispatched to
/// handleEnterNotify — and filtered, since suppression is still active —
/// strictly before this function ever runs.
...
/// EnterNotify suppression. Safe to call when no query is pending.
///
/// Only clears suppression when it is still .tiling_operation, so this
/// never clobbers a different reason set by something else in the meantime
/// (e.g. a window_spawn that raced in before this reply arrived).
pub fn drainTilingOpSettle() void {
    const cookie = state.tiling_op_cookie orelse return;
    state.tiling_op_cookie = null;
    const cs = core.getState();
    // Reply must be consumed to drain the XCB queue, even though its
    // content (whatever currently has X input focus) is unused — only its
    // arrival, which is what tells us the server has processed everything
    // queued before it, matters here.
