...
fn eqModel(a: *const Model, b: *const Model) bool {
    ...
    for (&a.ws, &b.ws) |*sa, *sb| {
        if (!std.mem.eql(WindowId, sa.tiled_order.items, sb.tiled_order.items)) return false;
        if (!std.mem.eql(WindowId, sa.focus_mru.items, sb.focus_mru.items)) return false;
...
test "T12: unregister cleans all references" {
    ...
    for (&m.ws) |*s| {
        try testing.expect(model.findInOrder(&s.focus_mru, 1) == null);
...
test "T15: focus MRU ordering and cap" {
    ...
    const mru = m.ws[0].focus_mru.items;
    ...
    try testing.expectEqual(wins[10], m.ws[0].focus_mru.items[0]);
    try testing.expectEqual(@as(usize, model.mru_capacity), m.ws[0].focus_mru.items.len);
...
test "T17: capacity refusals happen before any mutation" {
    ...
    for (&wins, 0..) |*w, i| {
        w.* = @intCast(200 + i);
        model.register(&ref, w.*, if (i == wins.len - 1) null else 0);
    ...
    // register rollback: OOM during list append undoes the store insert.
    var failing = testing.FailingAllocator.init(testing.allocator, .{ .fail_index = 0 });
    var fm = Model{ .gpa = failing.allocator() };
    defer {
        for (&fm.ws) |*s| {
            s.tiled_order.deinit(testing.allocator);
            s.focus_mru.deinit(testing.allocator);
        }
    }
    model.register(&fm, 500, null);
    try testing.expect(!fm.store.has(500));
}
...
test "T18: identical operation sequences produce identical models" {
    const seq = struct {
        fn run(m: *Model) void {
            for ([_]WindowId{ 1, 2, 3, 4, 5 }) |w| model.register(m, w, null);
            model.register(m, 6, 2);
            model.switchTo(m, 1);
            model.register(m, 7, null);
            model.switchTo(m, 0);
            model.reorderTiled(m, 3, 0);
            model.swapMaster(m);
            model.minimize(m, 4) catch unreachable;
