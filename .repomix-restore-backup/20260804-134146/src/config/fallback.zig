...
// Tests
//
// These deliberately use well-known, virtually-always-present real paths
// (rather than a temp file + chmod) to exercise checkPath's executable-bit
// check, since this codebase's Io abstraction (std.Io.Dir, used elsewhere in
// this file) is new enough that guessing at a temp-directory/chmod API this
// review can't verify against it would be riskier than it's worth.

const testing = std.testing;

test "checkPath rejects a path that exists but isn't executable (item 13)" {
    // /etc/passwd is present on essentially every Linux system and is
    // virtually never executable — standing in for the "stray non-executable
    // file happens to share a terminal's name" scenario item 13 describes.
    var buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    try testing.expect(!checkPath(&buf, "/etc", "passwd"));
}

test "checkPath accepts a path that exists and is executable (item 13)" {
    // /bin/sh (or a symlink to it) is present and executable on essentially
    // every Linux system, including CI containers.
    var buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    try testing.expect(checkPath(&buf, "/bin", "sh"));
}

test "checkPath rejects a nonexistent path" {
    var buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    try testing.expect(!checkPath(&buf, "/nonexistent-dir-xyz", "nonexistent-cmd-xyz"));
}
