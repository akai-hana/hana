...
test "KeybindResolver reflects the current build's actions across two rebuilds (item 10)" {
    // Simulates the reload sequence (config.load, then events.applyConfig on
    // every reload): rebuildDispatchMap is exercised directly here rather
    // than through `build`, since `build`'s keycode-resolution step needs a
    // live XkbState/X connection that a unit test doesn't have — see
    // rebuildDispatchMap's doc comment.
    var resolver: KeybindResolver = .{};
    defer resolver.deinit(testing.allocator);

    var first = [_]Keybind{
        .{ .modifiers = 1, .keysym = 100, .action = .close_window },
    };
    resolver.rebuildDispatchMap(&first, testing.allocator);
    try testing.expectEqual(.close_window, std.meta.activeTag(resolver.lookup(1, 100).?.*));
    try testing.expect(resolver.lookup(2, 200) == null);

    var second = [_]Keybind{
        .{ .modifiers = 1, .keysym = 100, .action = .toggle_layout },
        .{ .modifiers = 2, .keysym = 200, .action = .toggle_fullscreen },
    };
    resolver.rebuildDispatchMap(&second, testing.allocator);
    // Must reflect the *second* build, not a stale entry from the first —
    // this is the exact scenario the old bare-global g_keybind_map handled
    // correctly too, but only because of a hand-maintained defer ordering in
    // main.zig; this test exercises the resolver in isolation regardless of
    // that ordering.
    try testing.expectEqual(.toggle_layout, std.meta.activeTag(resolver.lookup(1, 100).?.*));
    try testing.expectEqual(.toggle_fullscreen, std.meta.activeTag(resolver.lookup(2, 200).?.*));
}

test "KeybindResolver keeps the later of two conflicting bindings (item 10)" {
    var resolver: KeybindResolver = .{};
    defer resolver.deinit(testing.allocator);
    ...
    resolver.rebuildDispatchMap(&kbs, testing.allocator);
    try testing.expectEqual(.toggle_layout, std.meta.activeTag(resolver.lookup(1, 100).?.*));
}

test "Config.deinit tears down keybind_resolver before freeing keybindings, without crashing" {
    var cfg: Config = .{};
    try cfg.keybindings.append(testing.allocator, .{ .modifiers = 1, .keysym = 100, .action = .close_window });
    cfg.keybind_resolver.rebuildDispatchMap(cfg.keybindings.items, testing.allocator);
    try testing.expect(cfg.keybind_resolver.lookup(1, 100) != null);

    // These fields are always heap-duped by parseBar in the real load path;
    // dupe them here too so Config.deinit's unconditional frees are valid.
    cfg.bar.font = try testing.allocator.dupe(u8, cfg.bar.font);
    cfg.bar.clock_format = try testing.allocator.dupe(u8, cfg.bar.clock_format);
    cfg.bar.drun_prompt = try testing.allocator.dupe(u8, cfg.bar.drun_prompt);
    cfg.bar.indicator_focused = try testing.allocator.dupe(u8, cfg.bar.indicator_focused);
    cfg.bar.indicator_unfocused = try testing.allocator.dupe(u8, cfg.bar.indicator_unfocused);

    cfg.deinit(testing.allocator); // must not crash / leak / use-after-free
}

test "lowerStringCI lowercases values that fit and reports too_long for values that don't (item 11)" {
    switch (lowerStringCI(4, "AbCd")) {
        .too_long => try testing.expect(false),
        .ok => |r| try testing.expectEqualStrings("abcd", r.slice()),
    }
    switch (lowerStringCI(4, "TooLong")) {
        .too_long => {},
        .ok => try testing.expect(false),
    }
    // Exactly at the limit is not too_long (boundary check).
    switch (lowerStringCI(3, "ABC")) {
        .too_long => try testing.expect(false),
        .ok => |r| try testing.expectEqualStrings("abc", r.slice()),
    }
}

test "fromStringCI is case-insensitive and rejects overlong input (item 11)" {
    try testing.expectEqual(MasterSide.left, fromStringCI(MasterSide, "LEFT").?);
    try testing.expectEqual(MasterSide.right, fromStringCI(MasterSide, "Right").?);
    try testing.expect(fromStringCI(MasterSide, "this-is-a-way-too-long-side-name-to-fit-in-32-bytes") == null);
    try testing.expect(fromStringCI(MasterSide, "diagonal") == null);
}
